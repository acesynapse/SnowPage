...
//        }
    ...
    public function imageUrl($number) {
      ...
      $sizearray = array();
      $headers = array_map('strtolower', @get_headers($urlone));
      $keysearch = 'content-type: image/jpeg';
      if(in_array($keysearch, $headers)) {
        $sizearray[$urlone] = getimagesize($urlone)[1];
      }
      $headers = array_map('strtolower', @get_headers($urltwo));
      $keysearch = 'content-type: image/jpeg';
      if(in_array($keysearch, $headers)) {
        $sizearray[$urltwo] = getimagesize($urltwo)[1];
      }
      $headers = array_map('strtolower', @get_headers($urlthree));
      $keysearch = 'content-type: image/jpeg';
      if(in_array($keysearch, $headers)) {
        $sizearray[$urlthree] = getimagesize($urlthree)[1];
      }
      $headers = array_map('strtolower', @get_headers($urlfour));
      $keysearch = 'content-type: image/jpeg';
      if(in_array($keysearch, $headers)) {
        $sizearray[$urlfour] = getimagesize($urlfour)[1];
      }
      $headers = array_map('strtolower', @get_headers($urlfive));
      $keysearch = 'content-type: image/jpeg';
      if(in_array($keysearch, $headers)) {
