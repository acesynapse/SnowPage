...
/* v2.0.0
/* Customized for SnowPage by AceSynapse
/* ----------------------------------------------- */
function hexToRgb(b) {
...
var pJS = function(b, c) {
  var d = document.querySelector("#" + b),
    e = document.querySelector("#" + b + " > .particles-js-canvas-el");
  this.pJS = {
    canvas: {
      el: e,
      w: d.offsetWidth,
  ...
  c && Object.deepExtend(a, c), a.tmp.obj = {
    ...
    if (this.color.rgb) var e = "rgba(" + this.color.rgb.r + "," + this.color.rgb.g + "," + this.color.rgb.b + "," + c + ")";
    else var e = "hsla(" + this.color.hsl.h + "," + this.color.hsl.s + "%," + this.color.hsl.l + "%," + c + ")";
    switch (a.canvas.ctx.fillStyle = e, a.canvas.ctx.beginPath(), this.shape) {
      case "circle":
      ...
      case "edge":
        ...
        break;
      case "triangle":
        a.fn.vendors.drawShape(a.canvas.ctx, this.x - b, this.y + b / 1.66, 2 * b, 3, 2);
        break;
      case "polygon":
        a.fn.vendors.drawShape(a.canvas.ctx, this.x - b / (a.particles.shape.polygon.nb_sides / 3.5), this.y - b / .76, 2.66 * b / (a.particles.shape.polygon.nb_sides / 3), a.particles.shape.polygon.nb_sides, 1);
        break;
      case "star":
        a.fn.vendors.drawShape(a.canvas.ctx, this.x - 2 * b / (a.particles.shape.polygon.nb_sides / 4), this.y - b / 1.52, 2 * b * 2.66 / (a.particles.shape.polygon.nb_sides / 3), a.particles.shape.polygon.nb_sides, 2);
        break;
      case "image":
        if ("svg" == a.tmp.img_type) var d = this.img.obj;
        else var d = a.tmp.img_obj;
        d && a.canvas.ctx.drawImage(d, this.x - b, this.y - b, 2 * b, 2 * b / this.img.ratio)
    }
    ...
    var c = a.x - b.x,
      d = a.y - b.y;
    a.radius + b.radius >= Math.sqrt(c * c + d * d) && (a.vx = -a.vx, a.vy = -a.vy, b.vx = -b.vx, b.vy = -b.vy)
    ...
    if (a.interactivity.events.onhover.enable && isInArray("repulse", a.interactivity.events.onhover.mode) && "mousemove" == a.interactivity.status) {
      ...
      if (a.tmp.repulse_finish || (a.tmp.repulse_count++, a.tmp.repulse_count == a.particles.array.length && (a.tmp.repulse_finish = !0)), a.tmp.repulse_clicking) {
        ...
        d >= l && function() {
          ...
          if (b.vx = m * Math.cos(d), b.vy = m * Math.sin(d), "bounce" == a.particles.move.out_mode) {
            var c = {
              x: b.x + b.vx,
              y: b.y + b.vy
            };
    ...
    var c = a.tmp.source_svg.replace(/#([0-9A-F]{3,6})/gi, function(b, c, d, e) {
        if (g.color.rgb) var a = "rgba(" + g.color.rgb.r + "," + g.color.rgb.g + "," + g.color.rgb.b + "," + g.opacity + ")";
        else var a = "hsla(" + g.color.hsl.h + "," + g.color.hsl.s + "%," + g.color.hsl.l + "%," + g.opacity + ")";
        return a
      }),
      d = new Blob([c], {
    ...
    b.addEventListener("load", function() {
      g.img.obj = b, g.img.loaded = !0, e.revokeObjectURL(f), a.tmp.count_svg++
    }), b.src = f
  }, a.fn.vendors.destroypJS = function() {
    cancelAnimationFrame(a.fn.drawAnimFrame), e.remove(), pJSDom = null
    ...
    for (var f = 0; i > f; f++) a.lineTo(b, 0), a.translate(b, 0), a.rotate(j);
    a.fill(), a.restore()
...
Object.deepExtend = function(c, b) {
  ...
  b.className = f, b.style.width = "100%", b.style.height = "100%", b.style.position = "absolute", b.style.top = "0", null != document.getElementById(a).appendChild(b) && pJSDom.push(new pJS(a, d))
